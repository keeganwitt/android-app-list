...
class SummaryCalculator(private val context: Context) {
    ...
    fun calculate(apps: List<App>, field: AppInfoField): SummaryItem? {
        return when (field) {
            ENABLED -> {
            ...
            APK_SIZE -> calculateSizeSummary(APK_SIZE, apps.map { it.sizes.apkBytes })
            APP_SIZE -> calculateSizeSummary(APP_SIZE, apps.map { it.sizes.appBytes })
            DATA_SIZE -> calculateSizeSummary(DATA_SIZE, apps.map { it.sizes.dataBytes })
            CACHE_SIZE -> calculateSizeSummary(CACHE_SIZE, apps.map { it.sizes.cacheBytes })
            EXTERNAL_CACHE_SIZE -> calculateSizeSummary(EXTERNAL_CACHE_SIZE, apps.map { it.sizes.externalCacheBytes })
            TOTAL_SIZE -> calculateSizeSummary(TOTAL_SIZE, apps.map { it.sizes.totalBytes })
            TARGET_SDK -> calculateSdkSummary(TARGET_SDK, apps.mapNotNull { it.targetSdk })
            MIN_SDK -> calculateSdkSummary(MIN_SDK, apps.mapNotNull { it.minSdk })
            GRANTED_PERMISSIONS -> calculatePermissionSummary(GRANTED_PERMISSIONS, apps.mapNotNull { it.grantedPermissionsCount })
            REQUESTED_PERMISSIONS -> calculatePermissionSummary(REQUESTED_PERMISSIONS, apps.mapNotNull { it.requestedPermissionsCount })
            VERSION, FIRST_INSTALLED, LAST_UPDATED, LAST_USED, PACKAGE_MANAGER -> null
        }
    }
