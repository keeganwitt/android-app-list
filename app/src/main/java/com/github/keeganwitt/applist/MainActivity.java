...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener, AppInfoAdapter.OnClickListener {
    ...
    private void initiateExport(String type) {
        if (appInfoAdapter == null || appInfoAdapter.getItemCount() == 0) {
            Toast.makeText(this, getString(R.string.export_no_apps), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            ...
            if (outputStream != null) {
                ...
                for (int i = 0; i < appInfoAdapter.getItemCount(); i++) {
                    ...
                    try {
                        infoValue = Html.escapeHtml(app.getTextValue(this, packageManager, usageStatsManager));
                    } catch (PackageManager.NameNotFoundException e) {
                        infoValue = "";
                    }
