...
class ApplicationInfoExtensionsTest {
    ...
    @Test
    @Config(sdk = [34])
    fun `given simulated SDK 35 and NoSuchFieldError, then isArchivedApp returns false`() {
        val originalSdk = Build.VERSION.SDK_INT
        try {
            ReflectionHelpers.setStaticField(Build.VERSION::class.java, "SDK_INT", 35)
            val appInfo = ApplicationInfo()
            // appInfo is from SDK 34, so it doesn't have isArchived field.
            // Accessing isArchivedApp (compiled against >34) should trigger NoSuchFieldError internaly and catch it.
            assertFalse(appInfo.isArchivedApp)
        } finally {
